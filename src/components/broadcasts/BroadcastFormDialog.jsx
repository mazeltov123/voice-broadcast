  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{isEditing ? "Edit Broadcast" : "Create Broadcast"}</DialogTitle>
        </DialogHeader>
        <div className="space-y-5 py-2">
          ...
          <div className="border border-border rounded-lg p-3 space-y-3">
            ...
            {scheduleEnabled && (
              <div className="space-y-1.5">
                <Label className="text-xs text-muted-foreground">Send date & time (EST, UTC-5)</Label>
                <div className="relative">
                ...
                {form.scheduled_at && (
                  <p className="text-[11px] text-muted-foreground">
                    Will send on {utcToEasternLocal(easternLocalToUTC(form.scheduled_at)).replace('T', ' ')} EST
                  </p>
                )}
              </div>
            )}
